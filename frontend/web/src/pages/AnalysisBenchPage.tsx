...
const AnalysisBenchPage: React.FC = () => {
    ...
    return (
        <div className="flex h-screen">
            ...
            <main className="flex-1 p-6 bg-stone-200">
                ...
                {!loading && !error && active === "analysis" ? (
                    <TechniqueAnalysisPanelBench analysis={analysis} />
                ) : null}
            </main>
        </div>
    );
...
function TechniqueAnalysisPanelBench({ analysis }: { analysis: AnalysisPayload | null }) {
                ...
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <h2 className="text-xl font-semibold text-stone-900">
                            {analysis.exercise?.toUpperCase?.() || "BENCH PRESS ANALYSIS"}{" "}
                        ...
                        <p className="text-stone-700 mt-1">
                            {overall?.one_line_summary ||
                                "Focus on smooth eccentric, stable touchpoint, and a controlled J-curve toward the shoulders."}
                        </p>
                    </div>
                    ...
                    <div className="bg-green-50 rounded-lg p-3">
                        ...
                        <ul className="list-disc ml-5 text-green-900">
                            {(overall?.key_wins && overall.key_wins.length > 0
                                    ? overall.key_wins
                                    : [
                                        "Consistent touchpoint on the chest",
                                        "Solid scapular retraction and upper back tightness",
                                        "Good bar control on the way up",
                                    ]
                            ).map((w, i) => (
                                <li key={i}>{w}</li>
                            ))}
                    ...
                    <div className="bg-yellow-50 rounded-lg p-3">
                        ...
                        <ul className="list-disc ml-5 text-yellow-900">
                            {(overall?.key_fixes && overall.key_fixes.length > 0
                                    ? overall.key_fixes
                                    : [
                                        "Slow down the eccentric; keep tension",
                                        "Maintain the J-curve path (slightly toward the face on ascent)",
                                        "Drive with the legs and keep the chest high",
                                    ]
                            ).map((f, i) => (
                                <li key={i}>{f}</li>
                            ))}
                                    ...
                                    {r.features ? (
                                        <div>
                                            ...
                                            <div className="grid grid-cols-2 gap-1 mt-1 text-stone-700">
                                                {pickFeatureBench(r.features, "ecc_p95_vy_m_s")}
                                                {pickFeatureBench(r.features, "stall_count")}
                                                {pickFeatureBench(r.features, "jcurve_dx_cm")}
                                                {pickFeatureBench(r.features, "drift_x_cm")}
                                                {pickFeatureBench(r.features, "rms_x_cm")}
                                            </div>
                                        </div>
                                    ) : null}
                                </div>
...
function pickFeatureBench(features: Record<string, number>, key: string) {
    if (!(key in features)) return null;
    const v = features[key];

    if (key === "ecc_p95_vy_m_s") {
        return (
            <div className="flex items-center justify-between">
                <span className="text-stone-500">ecc_p95_vy_m_s</span>
    ...
    if (key === "stall_count") {
        return (
            <div className="flex items-center justify-between">
                <span className="text-stone-500">stall_count</span>
...
export default AnalysisBenchPage;
