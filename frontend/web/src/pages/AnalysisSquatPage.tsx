...
const AnalysisSquatPage: React.FC = () => {
    ...
    return (
        <div className="flex h-screen">
            ...
            <main className="flex-1 p-6 bg-stone-200">
                ...
                {!loading && !error && active === "analysis" ? (
                    <TechniqueAnalysisPanelSquat analysis={analysis} />
                ) : null}
            </main>
        </div>
    );
...
function TechniqueAnalysisPanelSquat({ analysis }: { analysis: AnalysisPayload | null }) {
                ...
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <h2 className="text-xl font-semibold text-stone-900">
                            {analysis.exercise?.toUpperCase?.() || "SQUAT ANALYSIS"}{" "}
                            {analysis.created_at ? <span className="text-stone-500 text-sm">({new Date(analysis.created_at).toLocaleString()})</span> : null}
                        </h2>
                        <p className="text-stone-700 mt-1">{overall?.one_line_summary || "Focus on depth, torso angle, and a vertical bar path over midfoot."}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div className="bg-green-50 rounded-lg p-3">
                        ...
                        <ul className="list-disc ml-5 text-green-900">
                            {(overall?.key_wins && overall.key_wins.length > 0
                                    ? overall.key_wins
                                    : ["Consistent depth and balanced stance", "Controlled descent and stable midfoot pressure"]
                            ).map((w, i) => (
                                <li key={i}>{w}</li>
                            ))}
                        </ul>
                    ...
                    <div className="bg-yellow-50 rounded-lg p-3">
                        ...
                        <ul className="list-disc ml-5 text-yellow-900">
                            {(overall?.key_fixes && overall.key_fixes.length > 0
                                    ? overall.key_fixes
                                    : ["Keep chest up; brace hard", "Knees track over toes (avoid caving)", "Keep bar path vertical over midfoot"]
                            ).map((f, i) => (
                                <li key={i}>{f}</li>
                            ))}
                        </ul>
                    ...
                    return (
                        <div key={r.index} className="bg-white rounded-xl p-4 shadow">
                            ...
                            {issues.length > 0 ? (
                                ...
                                /* Fallback: heuristics flags + key squat features */
                                    ...
                                    {r.features ? (
                                        <div>
                                            ...
                                            <div className="grid grid-cols-2 gap-1 mt-1 text-stone-700">
                                                {pickFeatureSquat(r.features, "depth_ok")}
                                                {pickFeatureSquat(r.features, "torso_angle_bottom_deg")}
                                                {pickFeatureSquat(r.features, "jcurve_dx_cm")}
                                                {pickFeatureSquat(r.features, "drift_x_cm")}
                                                {pickFeatureSquat(r.features, "rms_x_cm")}
                                                {pickFeatureSquat(r.features, "stall_count")}
                                                {pickFeatureSquat(r.features, "bar_over_ankle_start_cm")}
                                                {pickFeatureSquat(r.features, "bar_over_ankle_lock_cm")}
                                                {pickFeatureSquat(r.features, "shoulder_bar_offset_start_cm")}
                                            </div>
                                        </div>
                                    ) : null}
                                </div>
...
function pickFeatureSquat(features: Record<string, number>, key: string) {
    ...
    if (key === "depth_ok") {
        ...
        return (
            <div className="flex items-center justify-between">
                <span className="text-stone-500">depth_ok</span>
...
export default AnalysisSquatPage;
